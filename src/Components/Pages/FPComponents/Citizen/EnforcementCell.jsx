...
const EnforcementCell = () => {
  ...
  const getUserList = () => {
    ...
    AxiosInterceptors
      .post(url, {userType : "EC"}, ApiHeader())
      .then((res) => {
        if (res?.data?.status) {
    ...
    <>
      ...
      <div className="poppins p-4 px-6">
        ...
        <div className="uppercase font-semibold text-gray-700 text-2xl py-2 text-center tracking-[0.5rem]">
          Know Your Enforcement Cell 
        </div>

        <div className="w-full h-[0.15rem] bg-gray-400 mb-6"></div>

        {/* <div className="flex gap-1 w-full flex-wrap my-6">
          <button onClick={() => setMType('user')} className={tabButtonStyle(mType == 'user')}>Enforcement Cell</button>
...
export default EnforcementCell;